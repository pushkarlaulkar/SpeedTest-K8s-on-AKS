...
To install this app using Helm using the default **App Routing** add on, perform below steps
  ...
  2. Run the command ` kubectl -n app-routing-system get svc nginx ` to confirm if a **LoadBalancer** IP has been provisioned.
     ...
     nginx   LoadBalancer   10.0.58.180   20.174.45.64   80:32767/TCP,443:30598/TCP   110s
     ```
  3. Run the command to install **SpeedTest**

     ```
     helm install speedtest ./helm --namespace speedtest --set ingressClassName="web-app-routing" --set domain_name=your_preferred_fqdn --set-file tlsSecret.cert=domain_name.crt --set-file tlsSecret.key=domain_name.key
     ```
  4. Run `kubectl -n speedtest get ingress` to retrieve the IP. This may take some time to match with the **LoadBalancer** IP above. Point the domain name in your registrar to the IP address.
...
To install this app using Helm using your own nginx ingress, perform below steps
  ...
  4. Run the command to install **SpeedTest**
     ...
     ```
     helm install speedtest ./helm --namespace speedtest --set ingressClassName="nginx" --set domain_name=your_preferred_fqdn --set-file tlsSecret.cert=domain_name.crt --set-file tlsSecret.key=domain_name.key
     ```
